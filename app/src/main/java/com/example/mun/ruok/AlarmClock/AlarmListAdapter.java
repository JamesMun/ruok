package com.example.mun.ruok.AlarmClock;
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		btnToggle.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				...
				AlarmModel mModel = AlarmFragment.dbHelper.getAlarm(((Long) buttonView.getTag()).longValue());
				mModel.isEnabled = isChecked;
				AlarmFragment.dbHelper.updateAlarm(mModel);

				AlarmManagerHelper.setAlarms(mContext);

				//mAlarmFragment.setAlarmEnabled(((Long) buttonView.getTag()).longValue(), isChecked);
			}
		});
		...
		view.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				MainActivity.UserActContext.startActivityForResult(intent,0);
				//mAlarmFragment.startAlarmDetailsActivity(((Long) view.getTag()).longValue());
			}
		});
		...
		view.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View view) {
				...
				final long alarmId = ((Long) view.getTag()).longValue();
				...
				builder.setMessage("Please confirm")
						...
						.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								...
								AlarmFragment.dbHelper.deleteAlarm(alarmId);
								//Refresh the list of the alarms in the adaptor
								AlarmFragment.mAdapter.setAlarms(AlarmFragment.dbHelper.getAlarms());
								//Notify the adapter the data has changed
								AlarmFragment.mAdapter.notifyDataSetChanged();
								//Set the alarms
								AlarmManagerHelper.setAlarms(mContext);
							}
						}).show();

				//mAlarmFragment.deleteAlarm(((Long) view.getTag()).longValue());
				return true;
			}
		});
